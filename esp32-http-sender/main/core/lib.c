...
{   
	...
	z->isEnable[13] = isEnable_mlx90614;
    ...
    z->s_name[10] = "Y";
    z->s_name[11] = "Z";
	z->s_name[12] = "SoilHumidity";
	z->s_name[13] = "BodyTemperature";
...
void getData(struct device *z, long *c)
{
	...
	if (check_mlx90614())
    {
        for (i = 0; i < 1; i++)
        {
            strcpy(z->d[i + 13], get_mlx90614(i));
        }
    }
    else
    {
...
void generateJson(struct device *z)
{
    ...
    {
        aux = 0;
        strcat(z->json, ",{\"sensor\":\"Soil\",\"data\":[");
        for (i = 0; i < 1; i++)
        {
            if (z->isEnable[i + 12])
            {
        ...
        for (i = 0; i < 1; i++)
        {
            if (z->isEnable[i + 13])
            {
                if (aux != i)
                    strcat(z->json, ",");
                strcat(z->json, "{\"");
                strcat(z->json, z->s_name[i + 13]);
                strcat(z->json, "\":\"");
                strcat(z->json, z->d[i + 13]);
                strcat(z->json, "\"}");
            }
            else
                aux++;
